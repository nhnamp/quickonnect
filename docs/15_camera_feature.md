...
The client now explicitly checks and requests Qt camera permission before starting `QCamera`. This is required on macOS; without the permission request, Qt can detect the `MacBook Pro Camera` device but then fail with `Access to camera not granted`. If macOS denies access, the app shows a message pointing the user to System Settings > Privacy & Security > Camera.
...
| `client/features/camera_engine.py` | Created / Modified | Captures camera frames with QtMultimedia, requests camera permission, downscales/encodes JPEG frames, and sends them through the connection manager. |
...
| `tests/test_camera_engine.py` | Created / Modified | Covers camera image scaling, JPEG decode/encode helpers, and the macOS permission-denied guidance. |
...
QtMultimedia is already available through PyQt6 in this project, so no new OpenCV dependency is required. On macOS and Linux, camera access still depends on operating-system camera permissions and an available video input device. macOS may require restarting the app after changing camera permissions in System Settings.
...
8. Test on macOS with camera permission unset and confirm the app prompts for permission before starting video.
9. Test on macOS and Linux with OS camera permissions denied once to confirm the app shows a clear startup error.
